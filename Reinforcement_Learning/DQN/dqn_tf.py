...
class DeepQNetwork(object):
    ...
    def __init__(self, lr, n_actions, name, fcl_dims = 1024,
    ...
    def build_network(self):
        with tf.variable_scope(self.name):
            ...
            densel = tf.layers.dense(flat, units=self.fcl_dims,
                                     activation=tf.nn.relu,
                        kernel_initializer=tf.variance_scaling_initializer(scale=2))

            self.Q_values = tf.layers.dense(densel, units=self.n_act,
                        kernel_initializer=tf.variance_scaling_initializer(scale=2))
...
class Agent(object):
    ...
    def store_transitions(self, state, action, reward, state_, terminal):
    ...
    def learn(self):
        ...
        _ = self.q_eval.sess.run(self.q_eval.train_op,
                        feed_dict={self.q_eval.input: state_batch,
                                   ...
                                   self.q_eval.q_tareget: q_target})

        if self.mem_cntr > 25000:
